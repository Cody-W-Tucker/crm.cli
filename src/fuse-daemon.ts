...
// ── Enriched JSON builders (reused from fuse.ts logic) ──
...
async function buildCompanyJSON(
  db: DB,
  co: (typeof schema.companies)['$inferSelect'],
): Promise<Record<string, unknown>> {
...
async function buildDealJSON(
  db: DB,
  d: (typeof schema.deals)['$inferSelect'],
): Promise<Record<string, unknown>> {
  ...
  // Always include initial stage entry at deal creation time
  const stageHistory = [
...
function buildActivityJSON(
  a: (typeof schema.activities)['$inferSelect'],
...
const CONTACT_WRITE_FIELDS = new Set([
  ...
  'created_at',
  'updated_at',
  // Common custom field names accepted as top-level (stored in custom_fields)
  'title',
  'department',
  'source',
  'notes',
...
async function writeContact(
  ...
  for (const key of Object.keys(data)) {
    ...
    // Common custom field names get moved to custom_fields
    if (
      [
        'title',
        'department',
