...
describe('hooks', () => {
  test('post-contact-add hook receives entity JSON', () => {
    ...
    ctx.runOK('--config', configPath, 'contact', 'add', '--name', 'Jane', '--email', 'jane@acme.com')
  ...
  test('pre-contact-rm hook can abort deletion', () => {
    ...
    const id = ctx.runOK('--config', configPath, 'contact', 'add', '--name', 'Jane').trim()
  ...
  test('post-deal-stage-change hook fires on move', () => {
    ...
    const id = ctx.runOK('--config', configPath, 'deal', 'add', '--title', 'Hook Deal', '--stage', 'lead').trim()
    ctx.runOK('--config', configPath, 'deal', 'move', id, '--stage', 'qualified')
