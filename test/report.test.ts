...
describe('report pipeline', () => {
  ...
  test('json format has stage/count/value fields', () => {
    ...
    const report = ctx.runJSON<Array<{ stage: string; count: number; value: number }>>('report', 'pipeline', '--format', 'json')
...
describe('report activity', () => {
  ...
  test('group by type', () => {
    ...
    const report = ctx.runJSON<unknown[]>('report', 'activity', '--by', 'type', '--format', 'json')
    expect(report.length).toBeGreaterThan(0)
  })
  ...
  test('group by contact', () => {
    ...
    const report = ctx.runJSON<unknown[]>('report', 'activity', '--by', 'contact', '--format', 'json')
  ...
  test('period filter', () => {
    ...
    const report = ctx.runJSON<Array<{ count: number }>>('report', 'activity', '--period', '7d', '--format', 'json')
...
describe('report stale', () => {
  test('flags contacts with no activity', () => {
    const ctx = createTestContext()
    ctx.runOK('contact', 'add', '--name', 'Active Jane', '--email', 'jane@acme.com')
...
describe('report conversion', () => {
  test('shows stage conversion rates', () => {
    ...
    const deals = ctx.runJSON<Array<{ id: string }>>('deal', 'list', '--stage', 'lead', '--format', 'json')
    for (let i = 0; i < 3; i++) {
  ...
  test('json format', () => {
    ...
    const report = ctx.runJSON<unknown[]>('report', 'conversion', '--format', 'json')
    expect(report.length).toBeGreaterThan(0)
  })
})
...
describe('report velocity', () => {
  test('shows time per stage', () => {
    const ctx = createTestContext()
    const id = ctx.runOK('deal', 'add', '--title', 'Fast Deal', '--stage', 'lead').trim()
...
describe('report forecast', () => {
  test('shows weighted forecast', () => {
    const ctx = createTestContext()
    ctx.runOK('deal', 'add', '--title', 'Deal A', '--value', '50000', '--probability', '80', '--expected-close', '2026-06-15')
    ctx.runOK('deal', 'add', '--title', 'Deal B', '--value', '30000', '--probability', '50', '--expected-close', '2026-06-20')
  ...
  test('period filter', () => {
    const ctx = createTestContext()
    ctx.runOK('deal', 'add', '--title', 'Q2 Deal', '--value', '50000', '--expected-close', '2026-06-15')
    ctx.runOK('deal', 'add', '--title', 'Q3 Deal', '--value', '30000', '--expected-close', '2026-09-15')

    const report = ctx.runJSON<unknown[]>('report', 'forecast', '--period', '2026-06', '--format', 'json')
...
describe('report won/lost', () => {
  test('report won shows closed-won deals', () => {
    const ctx = createTestContext()
    const id = ctx.runOK('deal', 'add', '--title', 'Won Deal', '--value', '25000', '--stage', 'lead').trim()
    ctx.runOK('deal', 'move', id, '--stage', 'closed-won')
  ...
  test('report lost shows reasons', () => {
    const ctx = createTestContext()
    const id = ctx.runOK('deal', 'add', '--title', 'Lost Deal', '--value', '15000', '--stage', 'lead').trim()
    ctx.runOK('deal', 'move', id, '--stage', 'closed-lost', '--reason', 'Too expensive')
  ...
  test('report won with period', () => {
    ...
    const report = ctx.runJSON<unknown[]>('report', 'won', '--period', '30d', '--format', 'json')
    expect(report).toHaveLength(1)
  })
})
